...
from predcoding.training import train_fptt_supervised, get_stats_named_params, reset_named_params
from predcoding.experiments.eval import test_clf
...
def main():
    ...
    Path(f"./images/reconstructions/{file_name}/{timestamp}").mkdir(parents=True, exist_ok=True)
    ...
    for epoch in tqdm(range(epochs), total=epochs):
        ...
        model.eval()
        test_loss, test_acc = test_clf(model, test_loader, T)
        print(f"Test loss: {test_loss:.4f} | test acc: {100.0 * test_acc:.2f}")
    ...
    test_losses_img_path = f"./images/test_losses/{file_name}/{timestamp}"
